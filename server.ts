...
app.post("/api/upload", (req, res, next) => {
  ...
  upload.single("pdf")(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      ...
      return res.status(500).json({ error: `Erro interno no servidor durante o upload: ${err.message}` });
    }
    
    const file = req.file;
    if (!file) {
      console.warn("Upload finalizado mas nenhum arquivo foi encontrado no request.");
      return res.status(400).json({ error: "Nenhum arquivo enviado." });
    }

    console.log(`Upload concluído com sucesso: ${file.filename} (${file.size} bytes)`);
    res.json({ 
      filePath: file.path,
